...
#define MAX_ANGULAR_VELOCITY 8.0f
...
float calculate_reward() {
    if (is_terminal_state()) {
        return -10.0f;  // Less extreme penalty
    }
    
    // Scale down other components
    float angle_reward = 1.0f - (my_abs(state.pole_angle) / MAX_ANGLE_RAD);
    angle_reward = angle_reward * angle_reward;  // Quadratic instead of quartic
    
    // Scale down position reward
    float position_reward = 1.0f - (my_abs(state.cart_position) / 2.0f);
    
    // Smaller combined reward
    return 5.0f * angle_reward + position_reward + 0.1f;  // Living bonus
}
